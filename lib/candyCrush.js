(function(){
  ...
  var animateScoreBoard = function(){
    $("#current-score-board").addClass("current-score-board-react");
    $("current-score-board").one("transitionend", function(){
      $("#current-score-board").removeClass("current-score-board-react");
    })
  };
  ...
  Game.prototype.detectClear = function(){
    ...
    for(var row = 1; row < thisGame.rowCount; row++){
      for(var col = 0; col < thisGame.colCount; col++){
        ...
        var comb = thisGame.findLongestHorizontal(row, col);
        if(comb.length >= 3){
        ...
        var comb = thisGame.findLongestVertical(row, col);
        if(comb.length >= 3){
          comb.forEach(function(candy){
            animateScoreBoard();
            thisGame.score += 10;
