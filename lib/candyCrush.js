(function(){
  ...
  Game.prototype.setoffBombs = function(){
    var thisGame = this;

    thisGame.gatherBombs();

    if(thisGame.bombs.length > 0){
      for(var index = 0; index < thisGame.bombs.length; index++){
        var bomb = thisGame.bombs[index];
        thisGame.blastRegion(bomb.row, bomb.col);
        thisGame.bombs.splice(0, 1);
      }
      return true;
    }else{
      return false;
  ...
  Game.prototype.blastRegion = function(row, col){
    var thisGame = this;

    for(var rowIdx = row - 1; rowIdx <= row + 1; rowIdx++){
      for(var colIdx = col - 1; colIdx <= col + 1; colIdx++){
        if(rowIdx < 0 || rowIdx > this.rowCount - 1 || colIdx < 0 || colIdx > this.colCount - 1){ continue; }

        thisGame.removeCandy(rowIdx, colIdx, true);
      }
    }
  };

  Game.prototype.gatherBombs = function(){
    var thisGame = this;

    thisGame.candies.forEach(function(candy){
      if(candy.bomb){
        thisGame.bombs.push(candy);
      };
    });
  };
  ...
  Game.prototype.removeCandy = function(row, col, burst){
    var targetCandy = this.findCandy(row, col);

    if(targetCandy && targetCandy.isCandy()){
      if(targetCandy.wrapped){
        targetCandy.becomeBomb();
        return 0;
      }
